...
def main_model(
	...
	for epoch in range(1, max_epochs + 1):
		for batch in batches_train:
			...
			if numpy.mod(n_batches, freq_disp) == 0:
				print('\tEPOCH: %6d    BATCH: %6d    COST: %12.6f' % (epoch, n_batches, cost))
			...
			if numpy.mod(n_batches, freq_val) == 0:
				...
				_, err_train = pred_error(f_prob, samples_train, get_batches(len(samples_train), batch_size_pred), mat_train)
	...
	print('TIME: %9.3f sec    EPOCHS: %6d    SPEED: %9.3f sec/epoch' % (time_end - time_start, epoch, (time_end - time_start) / epoch))
