...
def main_model(
    ...
    n_dim_txt=256,                      # Dimension of word embedding
    ...
    lrate=1,                            # Learning rate for optimizer
    ...
    # Flag of dropout
    use_dropout = theano.shared(to_t_float(0.), name='use_dropout')
    # Inputs
    ...
    # Word embedding
    out_emb = embedding(y, t_params, n_dim_vocab, n_dim_txt, 'emb').reshape((n_steps, n_samples, n_dim_txt))[1:]
    out_emb = tensor.concatenate([tensor.zeros((1, n_samples, n_dim_txt)), out_emb], axis=0)
    # Decoder(s)
    out_dec = gru(mask_y, tensor.concatenate([out_enc, out_emb], axis=2), t_params, n_dim_enc + n_dim_txt, n_dim_dec, 'dec_1')
    # Add dropout to the output of the final decoder
    out_dec = dropout(out_dec, use_dropout, trng)
    # Classifier
    out_fc = dense(out_dec, t_params, n_dim_dec, n_dim_vocab, 'fc_1')
    out_fc = tensor.nnet.softmax(out_fc.reshape((n_steps * n_samples, n_dim_vocab)))
    # Cost function
    ...
    f_prob = theano.function([x, mask_x, y, mask_y], out_fc.reshape((n_steps, n_samples, n_dim_vocab)), name='f_prob')
    ...
    while n_epochs < max_epochs:
        ...
        for batch in batches_train:
            ...
            if numpy.mod(n_batches, freq_disp) == 0:
                print('\tEPOCH: %6d    BATCH: %6d    COST: %12.6f' % (n_epochs, n_batches, cost))
...
if __name__ == '__main__':
    ...
    # main_model(max_samples_train=400, max_samples_val=50, max_samples_test=50, batch_size_train=50, batch_size_pred=50, max_epochs=100, freq_disp=1, freq_val=0, freq_save=0)
    main_model()
