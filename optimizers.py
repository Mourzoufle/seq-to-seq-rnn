...
def sgd(lrate, t_params, grads, inputs, cost):
    '''
    Stochastic Gradient Descent

    Parameters
    ----------
    lrate: Theano SharedVariable variable
    ...
    pup = [(p, p - lrate * g) for p, g in zip(t_params.values(), gshared)]
    # Function that updates the weights from the previously computed gradient
    f_update = theano.function([lrate], [], updates=pup, name='sgd_f_update')

    return f_cost, f_update


def adam(lrate, t_params, grads, inputs, cost):
    '''
    ...
    expon = theano.shared(to_floatX(0.), 'adam_expon')
    lrate *= tensor.sqrt(1 - 0.001 ** (expon + 1)) / (1 - 0.1 ** (expon + 1))
    m_up2 = [(m, 0.1 * g + 0.9 * m) for m, g in zip(m_up, gshared)]
    v_up2 = [(v, 0.001 * tensor.sqrt(g) + 0.999 * v) for v, g in zip(v_up, gshared)]
    param_up = [(p, p - lrate * m / (tensor.sqrt(v) + 1e-8)) for m, v, p, g in zip(m_up2, v_up2, t_params.values(), gshared)]
    f_update = theano.function([lrate], [], updates=m_up2 + v_up2 + param_up + (expon, expon + 1), on_unused_input='ignore', name='adam_f_update')

    return f_cost, f_update


def adadelta(lrate, t_params, grads, inputs, cost):
    '''
    An adaptive learning rate optimizer
    ...
    f_update = theano.function([lrate], [], updates=ru2up + param_up, on_unused_input='ignore', name='adadelta_f_update')

    return f_cost, f_update


def rmsprop(lrate, t_params, grads, inputs, cost):
    '''
    ...
    f_update = theano.function([lrate], [], updates=updir_new + param_up, on_unused_input='ignore', name='rmsprop_f_update')
