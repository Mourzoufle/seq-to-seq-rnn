...
def sgd(lr, t_params, grads, inputs, cost):
    ...
    gshared = [theano.shared(p.get_value() * 0., '%s_grad' % k) for k, p in t_params.iteritems()]
...
def adam(lr, t_params, grads, inputs, cost):
    ...
    '''
    gshared = [theano.shared(p.get_value() * to_t_float(0.), '%s_grad' % k) for k, p in t_params.iteritems()]
    m_up = [theano.shared(p.get_value() * to_t_float(0.), '%s_mup' % k) for k, p in t_params.iteritems()]
    v_up = [theano.shared(p.get_value() * to_t_float(0.), '%s_vup' % k) for k, p in t_params.iteritems()]

    gsup = [(gs, g) for gs, g in zip(gshared, grads)]
    f_cost = theano.function(inputs, cost, updates = gsup, name = 'adam_f_grad_shared')

    e = theano.shared(to_t_float(0.), 'adam_e')
...
def adadelta(lr, t_params, grads, inputs, cost):
    ...
    zipped_grads = [theano.shared(p.get_value() * to_t_float(0.), '%s_grad' % k) for k, p in t_params.iteritems()]
    running_up2 = [theano.shared(p.get_value() * to_t_float(0.), '%s_rup2' % k) for k, p in t_params.iteritems()]
    running_grads2 = [theano.shared(p.get_value() * to_t_float(0.), '%s_rgrad2' % k) for k, p in t_params.iteritems()]
...
def rmsprop(lr, t_params, grads, inputs, cost):
    ...
    zipped_grads = [theano.shared(p.get_value() * to_t_float(0.), '%s_grad' % k) for k, p in t_params.iteritems()]
    running_grads = [theano.shared(p.get_value() * to_t_float(0.), '%s_rgrad' % k) for k, p in t_params.iteritems()]
    running_grads2 = [theano.shared(p.get_value() * to_t_float(0.), '%s_rgrad2' % k) for k, p in t_params.iteritems()]
    ...
    updir = [theano.shared(p.get_value() * to_t_float(0.), '%s_updir' % k) for k, p in t_params.iteritems()]
