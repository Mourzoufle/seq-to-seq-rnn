...
def sgd(lr, t_params, grads, inputs, cost):
    ...
    gshared = [theano.shared(p.get_value() * 0., name='%s_grad' % k) for k, p in t_params.iteritems()]
    gsup = [(gs, g) for gs, g in zip(gshared, grads)]

    # Function that computes gradients for a mini-batch, but do not updates the weights.
    f_grad_shared = theano.function(inputs, cost, updates=gsup, name='sgd_f_grad_shared')
...
def adam(lr, t_params, grads, inputs, cost):
    ...
    '''
    gshared = [theano.shared(p.get_value() * to_t_float(0.), name='%s_grad' % k) for k, p in t_params.iteritems()]
    m_up = [theano.shared(p.get_value() * to_t_float(0.), name='%s_mup' % k) for k, p in t_params.iteritems()]
    v_up = [theano.shared(p.get_value() * to_t_float(0.), name='%s_vup' % k) for k, p in t_params.iteritems()]
    gsup = [(gs, g) for gs, g in zip(gshared, grads)]
    f_grad_shared = theano.function(inputs, cost, updates=gsup, name='adam_f_grad_shared')

    i = theano.shared(to_t_float(0.))
    lr *= tensor.sqrt(1 - 0.001 ** (i + 1)) / (1 - 0.1 ** (i + 1))
    ...
    i_up = (i, i + 1)
    f_update = theano.function([lr], [], updates=m_up2 + v_up2 + param_up + i_up, on_unused_input='ignore', name='adam_f_update')
...
def adadelta(lr, t_params, grads, inputs, cost):
    ...
    zipped_grads = [theano.shared(p.get_value() * to_t_float(0.), name='%s_grad' % k) for k, p in t_params.iteritems()]
    running_up2 = [theano.shared(p.get_value() * to_t_float(0.), name='%s_rup2' % k) for k, p in t_params.iteritems()]
    running_grads2 = [theano.shared(p.get_value() * to_t_float(0.), name='%s_rgrad2' % k) for k, p in t_params.iteritems()]
...
def rmsprop(lr, t_params, grads, inputs, cost):
    ...
    zipped_grads = [theano.shared(p.get_value() * to_t_float(0.), name='%s_grad' % k) for k, p in t_params.iteritems()]
    running_grads = [theano.shared(p.get_value() * to_t_float(0.), name='%s_rgrad' % k) for k, p in t_params.iteritems()]
    running_grads2 = [theano.shared(p.get_value() * to_t_float(0.), name='%s_rgrad2' % k) for k, p in t_params.iteritems()]
    zgup = [(zg, g) for zg, g in zip(zipped_grads, grads)]
    rgup = [(rg, 0.95 * rg + 0.05 * g) for rg, g in zip(running_grads, grads)]
    rg2up = [(rg2, 0.95 * rg2 + 0.05 * (g ** 2)) for rg2, g in zip(running_grads2, grads)]
    f_grad_shared = theano.function(inputs, cost, updates=zgup + rgup + rg2up, name='rmsprop_f_grad_shared')

    updir = [theano.shared(p.get_value() * to_t_float(0.), name='%s_updir' % k) for k, p in t_params.iteritems()]
